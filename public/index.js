...
var trackOtherDataList = [];

// Put all onload AJAX calls here, and event listeners
jQuery(document).ready(function() {

    $("#login-button").submit((e) => {
        e.preventDefault();
    ...
    // When the dropdown list of files has a changed selection
    $("#filedropdown").change((e) => {
        if($("#filedropdown").val() === "")
        {
    ...
    $('#otherdata-button').submit(function(e){
        ...
        {
            let component = $("input[name='component']:checked").attr('id');
            let index = new Number(component.slice((component.length - 5)*-1)) - 1;
            if (component.slice(0,5) === "Track")
            {
                ...
                if(trackOtherDataList[index].length === 0){
                    ...
                    trackOtherDataList[index].forEach((e) => {
                        let data = e.name + ": " + e.value + "\n";
                        dataString += data;
                    });
                }
                alert(dataString);
            ...
            else if (component.slice(0,5) === "Route")
            {
                ...
                if(routeOtherDataList[index].length === 0){
                    dataString += "No data to be shown for Route " + (index+1);
                }
                else{
                    dataString += "Other Data for Route " + (index+1) + ":\n";
                    routeOtherDataList[index].forEach((e) => {
                        let data = e.name + ": " + e.value + "\n";
                        dataString += data;
                    });
                }
    ...
    $("#between-button").submit(function(e){
        ...
        $.ajax({
            ...
            success: function({data}){
                ...
                let num = 1;
                data.forEach((file) => {
                    file.routes.routes.forEach((route) => {
                        $("#componentsBetween").append(        
                            "<div>" +
                            "<b>File</b>: " + file.routes.name +
                            " Route " + num++ +
                            "- <b>Name: </b>" + route.name +
                            ", <b># of Points: </b>" + route.numPoints + 
                            ", <b>Length: </b>" + route.len + "m" +
                            ", <b>Loop: </b>" + route.loop +
                            "</div>"
                        );
                    });
                ...
                num = 1;
                data.forEach((file) => {
                    file.tracks.tracks.forEach((track) => {
                        $("#componentsBetween").append(        
                            "<div>" +
                            "<b>File</b>: " + file.tracks.name +
                            " Route " + num++ +
                            "- <b>Name: </b>" + track.name +
                            ", <b># of Points: </b>" + track.numPoints + 
                            ", <b>Length: </b>" + track.len + "m" +
                            ", <b>Loop: </b>" + track.loop +
                            "</div>"
                        );
                    });
    ...
    $("#store-all-button").submit((e) => {
        e.preventDefault();

        clearAll();
        storeAll();
    });
    $("#clear-all-button").submit((e) => {
        e.preventDefault();
        clearAll();
        displayStatus();
    });
    $("#display-status-button").submit((e) => {
        e.preventDefault();
        displayStatus();
    });
    $("#execute-query-button").submit((e) => {
        e.preventDefault();
    });
});
    ...
    jQuery.ajax({
        ...
        success: function({data}) {
            if(data.length === 0){
                ...
                data.forEach(gpx => {
                    $('#filedropdown').append(
                        "<option value=\"" + gpx.name + "\">" + gpx.name.slice(0,-4) + "</option>"
                    );
    
                    $('#filelog').append(
                        "<tr>" +
                        "<td>" +
                        "<a href=\"" + gpx.name + "\" download \">" + gpx.name + "</a>" +
                        "</td>" +
                        "<td>"+ gpx.version +"</td>" +
                        "<td>" + gpx.creator + "</td>" +
        ...
        success: function({data}){
            if (JSON.stringify(data) != "{}"){                
                ...
                routes.forEach(route => {
                    ...
                    $("#gpxview").append(        
                        "<div>" +         
                        "<input type=\"radio\" name=\"component\" id=\"Route" + num + "\" value=\"" + route.route.name +"\">"+
                        "  <b>Route " + num++ + "</b>" +
                        "- <b>Name: </b>" + route.route.name +
                        ", <b># of Points: </b>" + route.route.numPoints + 
                        ", <b>Length: </b>" + route.route.len + "m" +
                        ", <b>Loop: </b>" + route.route.loop +
                        "</div>"
                    );
                });
                ...
                tracks.forEach(track => {
                    ...
                    $("#gpxview").append(                    
                        "<div>" +         
                        "<input type=\"radio\" name=\"component\" id=\"Track" + num + "\" value=\"" + track.track.name +"\">"+
                        "  <b>Track " + num++ + "</b>" +
                        " - <b>Name: </b>" + track.track.name +
                        ", <b># of Points: </b>" + track.track.numPoints + 
                        ", <b>Length: </b>" + track.track.len + "m" +
                        ", <b>Loop: </b>" + track.track.loop +
                        "</div>"
                    );
                });
            }
...
function addRouteHTML(){
    return (
        "<p>Adding route to " + $("#filedropdown").val() + "</p>" +
        "<p id='ptsJSON'>{\"name\":\"\",\"longitudes\":[],\"latitudes\":[]}</p>" +
        "<form ref='addPt-button' id='addPt-button'>" +
        "<div class=\"form-group\">" +
        "<input type=\"text\" id=\"routeName\"class=\"form-control\" value=\"\" placeholder=\"Route Name ...\">" +
        "<input type=\"text\" id=\"newRouteLon\"class=\"form-control\" value=\"\" placeholder=\"Longitude ...\">" +
        "<input type=\"text\" id=\"newRouteLat\"class=\"form-control\" value=\"\" placeholder=\"Latitude ...\">" +
        "</div>" +
        "<div class=\"form-group\">" +
        "<input type='submit' class=\"btn btn-secondary\" value=\"+ Add Point\">" +
        "</div>" +
        "</form>" +
        "<form ref='undo-button' id='undo-button'>" +
        "<div class=\"form-group\">" +
        "<input type='submit' class=\"btn btn-secondary\" value=\"- Undo\">" +
        "</div>" +
        "</form>" +
        "<form ref='addRt-button' id='addRt-button'>" +
        "<div class=\"form-group\">" +
        "<input type='submit' class=\"btn btn-secondary\" value=\"+ Add Route\">" +
        "</div>" +
        "</form>" +
        "<hr>"
    );
...
function addPoint(){
    if(!isNaN($("#newRouteLon").val()) && !isNaN($("#newRouteLat").val())){
        let points = JSON.parse($("#ptsJSON").text());
        let length = points.longitudes.length;
        points.longitudes[length] = new Number($("#newRouteLon").val());
        points.latitudes[length] = new Number($("#newRouteLat").val());
...
function addRoute(){
    if($("#filedropdown").val() === ""){
        ...
        let ptsJSON = JSON.parse($("#ptsJSON").text());
